...
export class GeneralHandler {
                ...
                project.getSourceFiles().forEach(srcFile => {
                    if (srcFile.getClasses().length > 0) {
                        srcFile.getClasses().forEach(aClass => {
                            this.data = task(aClass, this.data);
                        });
                    }
                });
            }
    ...
    addClassToMSE(clazz: ClassDeclaration, data:Data) {
                console.log("Here")
        console.log(data)

         data.parentClassId =  data.id;

         // let containerRef: number = this.getEntityContainerRef(clazz,data);
        let containerRef: number
        ...
        containerRef=  data.namespacesDictionary[namespaceDefinition];
    ...
    // getEntityContainerRef(clazz: ClassDeclaration,data:Data): number {
    //     var namespaceDefinition: string;
    //
    //     if (clazz.getParentNamespace() == undefined) {
    //         namespaceDefinition = "<Default Package>";
    ...
    addMethodToMSE(clazz: ClassDeclaration,data:Data) {
    ...
    addMethodsParametersToMSE(clazz: ClassDeclaration,data:Data) {
    ...
    addClassesAttributesToMSE(clazz: ClassDeclaration,data:Data) {
